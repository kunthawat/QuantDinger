...
def _build_telegram_report(
    ...
    def _render_pa(pa: Dict[str, Any], show_pnl: bool) -> None:
        ...
        # Show AI-suggested Entry / SL / TP when available (from trading_plan)
        tp = pa.get('take_profit') or pa.get('suggested_take_profit')
        sl = pa.get('stop_loss') or pa.get('suggested_stop_loss')
        entry = pa.get('entry_price') or pa.get('suggested_entry')
        if entry or sl or tp:
            entry_s = f"{entry:,.4f}" if entry else '—'
            sl_s = f"{sl:,.4f}" if sl else '—'
            tp_s = f"{tp:,.4f}" if tp else '—'
            lines.append(f"   📐 Entry: {entry_s}  SL: {sl_s}  TP: {tp_s}")
        reasoning = pa.get('reasoning', '')
        if reasoning:
